...
static statement_t* parse_define_statement(parser_t *p, int tok, bool assignable);
static statement_t* parse_define_statement2(parser_t *p, bool assignable);
...
static statement_t* parse_statement(parser_t *p) {
    ...
    case TOKEN_BYTE:{
        //handle byte[] ?
        res = parse_define_statement(p, p->lexer.cur_token.type, true);
    }break;

    case TOKEN_VAR: {
        res = parse_define_statement(p, TOKEN_DOUBLE, true);
        break;
    }
        case TOKEN_CONST: {
            res = parse_define_statement2(p, false);
            break;
        }
        case TOKEN_IF: {
...
static statement_t* parse_define_statement2(parser_t *p, bool assignable){
     ...
     return parse_define_statement(p, p->lexer.cur_token.type, assignable);
}
static statement_t* parse_define_statement(parser_t *p, int tok,
                                           bool assignable) {
    ...
    statement_t *res = statement_make_define(p->alloc, name_ident, tok,
                                             value, assignable);
    if (!res) {
        goto err;
    }
